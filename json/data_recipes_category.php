...
$sql = "SELECT recipes.*,categories.category_title AS category_title,chefs.chef_title AS chef_title FROM recipes,categories,chefs WHERE recipes.recipe_category = categories.category_id AND recipes.recipe_chef = chefs.chef_id AND recipes.recipe_status = 'Publish' AND recipes.recipe_category='".$_GET["category"]."' ORDER BY recipe_id DESC";
